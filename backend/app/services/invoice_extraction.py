...
def _infer_vendor_name(text: str) -> str | None:
    ...
    for line in text.splitlines():
        ...
        if GSTIN_PATTERN.search(value) or re.search(r"\b(?:invoice\s*(?:no|number|date)|gstin|bill\s*to|ship\s*to|subtotal|total|order\s*(?:no|number))\b", value, re.IGNORECASE):
            continue
        # In common invoice layouts the seller/company name is the first meaningful line.
        return value
    return None
...
def _extract_line_items(text: str) -> list[InvoiceLineItem]:
    """Extract invoice rows from common table and OCR layouts."""
    items: list[InvoiceLineItem] = []
    amount = r"[0-9][0-9,]*(?:\s*[0-9][0-9,]*)?(?:\.\s*[0-9]{1,2})?"

    def add_item(number: int, description: str, hsn: str, qty: str, amount_value: str) -> None:
        items.append(
            InvoiceLineItem(
                line_number=number,
    ...
    # Format 2: OCR stacks every table cell vertically.
    ...
    while index < len(lines):
        ...
        if index + 7 < len(lines):
            ...
            if (
                re.fullmatch(r"\d{4,8}", hsn)
                and _parse_number(qty) is not None
                and _parse_amount(unit_price) is not None
                and re.fullmatch(amount, discount_or_tax)
        ...
        if index + 5 < len(lines):
            description, hsn, qty, unit_price, amount_value = lines[index + 1:index + 6]
            if (
                re.fullmatch(r"\d{4,8}", hsn)
                and _parse_number(qty) is not None
                and _parse_amount(unit_price) is not None
                ...
                add_item(number, description, hsn, qty, amount_value)
                index += 6
                continue
