...
def get_document_text(document_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> ExtractedDocumentTextResponse:
    ...
    if extracted_text is None: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Extracted text not found. Process the document first.")
...
def extract_invoice(document_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> InvoiceExtractionResponse:
    ...
    return InvoiceExtractionResponse(document_id=document.id, fields=extract_invoice_fields_dict(extracted_text.extracted_text))
...
def invoice_confidence(document_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> InvoiceConfidenceResponse:
    ...
    extracted_text = db.scalar(select(ExtractedDocumentText).where(ExtractedDocumentText.document_id == document.id))
    if extracted_text is None: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Extracted text not found. Process the document first.")
    return InvoiceConfidenceResponse(document_id=document.id, **invoice_confidence_dict(extract_invoice_fields_dict(extracted_text.extracted_text)))
...
def evaluate_accuracy(document_id: str, payload: InvoiceAccuracyRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> InvoiceAccuracyResponse:
    ...
    extracted_text = db.scalar(select(ExtractedDocumentText).where(ExtractedDocumentText.document_id == document.id))
    if extracted_text is None: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Extracted text not found. Process the document first.")
    return InvoiceAccuracyResponse(document_id=document.id, **evaluate_invoice_accuracy(extract_invoice_fields_dict(extracted_text.extracted_text), payload.expected))
...
def process_document(document_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> DocumentProcessResponse:
    ...
    except DocumentFileNotFoundError as exc: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except UnsupportedDocumentTypeError as exc: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    except DocumentExtractionError as exc: raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(exc)) from exc
...
def analyze_document_endpoint(document_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> DocumentAnalysisResponse:
    document = db.scalar(select(Document).where(Document.id == document_id, Document.user_id == current_user.id))
    if document is None: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Document not found")
    try: outcome = analyze_document(document, db)
    except ExtractedTextNotFoundError as exc: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    except DocumentAnalysisError as exc: raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="AI analysis is temporarily unavailable. Extracted text is still available.") from exc
...
def get_document_analysis(document_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> DocumentAnalysisResponse:
    ...
    if analysis is None: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Analysis not found. Analyze the document first.")
