...
import type { DocumentRecord, InvoiceExtraction } from "../services/api";
...
function Detail({ doc, token, onClose }: { doc: DocumentRecord; token: string; onClose: () => void }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      else setError("Could not load extracted text. Process the document first.");
...
function InvoiceFields({ invoice, fallback }: { invoice: InvoiceExtraction | null; fallback: Awaited<ReturnType<typeof api.analysis>> }) {
  ...
  const money = (value: number | null | undefined) => value == null ? "N/A" : `${f?.currency || ""} ${value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`.trim();
...
function Field({ label, value }: { label: string; value: string | number | null | undefined }) { return <div className="field-item"><span>{label}</span><strong>{value === null || value === undefined || value === "" ? "N/A" : value}</strong></div>; }

function ResumeFields({ analysis }: { analysis: Awaited<ReturnType<typeof api.analysis>> }) { return <div className="extracted-fields"><h4>Extracted Fields</h4><div className="field-grid">{analysis.full_name && <Field label="Full Name" value={analysis.full_name} />}{analysis.date_of_birth && <Field label="Date of Birth" value={analysis.date_of_birth} />}{analysis.email && <Field label="Email" value={analysis.email} />}{analysis.phone && <Field label="Phone" value={analysis.phone} />}{analysis.education && <Field label="Education" value={analysis.education} />}{analysis.skills && <Field label="Skills" value={analysis.skills} />}{analysis.experience && <Field label="Experience" value={analysis.experience} />}{analysis.height && <Field label="Height" value={analysis.height} />}{analysis.father_name && <Field label="Father Name" value={analysis.father_name} />}{analysis.father_occupation && <Field label="Father Occupation" value={analysis.father_occupation} />}{analysis.mother_occupation && <Field label="Mother Occupation" value={analysis.mother_occupation} />}{analysis.siblings && <Field label="Siblings" value={analysis.siblings} />}</div></div>; }
